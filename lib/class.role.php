	...
	Class RoleDatabaseResultIterator extends DBCMySQLResult{
		public function current(){
			...
			$permissions = Symphony::Database()->query(
				"SELECT * FROM `tbl_uac_permissions` WHERE `role_id` = %d", array($role->id)
			);
	...
	Class RoleIterator implements Iterator{
	    ...
	    public function __construct(){
			$this->rows = Symphony::Database()->query(
				"SELECT * FROM `tbl_uac_roles` ORDER BY `name` ASC", NULL, 
	...
	Class Role{
		...
		public static function load($id){
			return Symphony::Database()->query(
				"SELECT * FROM `tbl_uac_roles` WHERE `id` = %d LIMIT 1", 
				array($id), 
				'RoleDatabaseResultIterator'
			)->current();
		}
		...
		public static function save(self $role, MessageStack &$errors){
			...
			elseif(
				Symphony::Database()->query("SELECT `id` FROM `tbl_uac_roles` WHERE `name` = '%s' %s",
				array(
					$role->name,
		...
		public static function delete($role_id, $replacement_role_id=NULL){
			...
			Symphony::Database()->delete('tbl_uac_roles', (array)$role_id, '`id` = %d');
		}
