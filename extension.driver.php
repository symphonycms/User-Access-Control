	...
	Class extensionUserAccessControl implements iExtension{
		...
		public function cbRemoveSectionPublishPermissions(array $context=NULL){
			Symphony::Database()->delete(
				'tbl_uac_permissions', array($context['handle']), "`key` = 'publish::%s'"
			);	
		}
		...
		public function cbSetDefaultPublishPermissions(array $context=NULL){
			if(isset($context['old-handle'])){
				...
				foreach(new RoleIterator as $role){
					Symphony::Database()->insert('tbl_uac_permissions', array(
						'id' => NULL,
						'role_id' => $role->id,
		...
		public function cbCheckPagePermissions(array $context=NULL){
			...
			elseif(preg_match('/^\/(blueprints|system)\/(datasource|events|views|sections|utilities)\//i', $context['callback']['pageroot'], $match)){
				$area = $match[1];
				$handle = $match[2];
		...
		private function modifyUsersPageIndex($context=NULL){
			...
			foreach($users as $position => $u){
				...
				$role = Symphony::Database()->query(
					sprintf(
						"SELECT r.name, r.id FROM `tbl_uac_roles` AS `r`
			...
			if($with_selected instanceof DOMNode){
				//$roles = Symphony::Database()->query("SELECT * FROM `tbl_uac_roles` ORDER BY `name` ASC");
...
*/
		...
		public function uninstall(){
			Symphony::Database()->query("DROP TABLE `tbl_uac_roles`");
			Symphony::Database()->query("DROP TABLE `tbl_uac_permissions`");
			Symphony::Database()->query("DROP TABLE `tbl_uac_sessions`");
			Symphony::Database()->query("DROP TABLE `tbl_uac_forgotpass`");
		...
		public function install(){
			...
			Symphony::Database()->query("CREATE TABLE `tbl_uac_roles` (
			...
			Symphony::Database()->query("CREATE TABLE `tbl_uac_permissions` (
			  `id` int(11) unsigned NOT NULL AUTO_INCREMENT,
			...
			Symphony::Database()->query("CREATE TABLE `tbl_uac_sessions` (
			...
			Symphony::Database()->query("CREATE TABLE `tbl_uac_forgotpass` (
			...
			// Default roles
			Symphony::Database()->insert('tbl_uac_roles', array('id' => 1, 'name' => 'Author'));
			Symphony::Database()->insert('tbl_uac_roles', array('id' => 2, 'name' => 'Developer'));
...
*/
	...
	return 'extensionUserAccessControl';